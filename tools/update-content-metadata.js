...
function buildEmbeddingState(existingRecord) {
  const defaults = {
    status: 'pending',
    model: null,
    vectorSha256: null,
  ...
  if (existingRecord && existingRecord.embedding && typeof existingRecord.embedding === 'object') {
    return { ...defaults, ...existingRecord.embedding };
  }
  return { ...defaults };
}

function manifestsMatchIgnoringGeneratedAt(existingManifest, nextManifest) {
  if (!existingManifest) {
    return false;
  }
  const serialize = (manifest) => JSON.stringify({
    meta: {
      ...manifest.meta,
      generatedAt: null,
    },
  ...
  return serialize(existingManifest) === serialize(nextManifest);
}

function finalizeManifest(manifest, existingManifest) {
  if (!existingManifest || !existingManifest.meta) {
    return manifest;
  }
  const previousGeneratedAt = existingManifest.meta.generatedAt;
  if (previousGeneratedAt && manifestsMatchIgnoringGeneratedAt(existingManifest, {
    meta: {
      ...manifest.meta,
    ...
    manifest.meta.generatedAt = previousGeneratedAt;
  }
  return manifest;
}

function buildJokesManifest(jokes, existingManifest) {
  const records = {};
  const existingRecords = existingManifest && existingManifest.records ? existingManifest.records : {};
  jokes.forEach((entry, index) => {
    ...
    const existingRecord = existingRecords[entry.id];
    records[entry.id] = {
      hashes: {
      ...
      embedding: buildEmbeddingState(existingRecord),
    };
  });
...
function buildQuotesManifest(categories, existingManifest) {
  const records = {};
  const existingRecords = existingManifest && existingManifest.records ? existingManifest.records : {};
  categories.forEach((category) => {
    category.quotes.forEach((quote, index) => {
      ...
      const combinedNormalized = combinedParts.join('|');
      const existingRecord = existingRecords[quote.id];
      records[quote.id] = {
        hashes: {
...
function main() {
  ...
  const existingJokesManifest = readJsonIfExists(jokesManifestPath);
  const existingQuotesManifest = readJsonIfExists(quotesManifestPath);

  writeJson(jokesManifestPath, buildJokesManifest(jokesWithIds, existingJokesManifest));
  writeJson(quotesManifestPath, buildQuotesManifest(quotesWithIds, existingQuotesManifest));
