...
- Run `node tools/update-content-metadata.js` after refreshing the jokes or quotes datasets to assign deterministic IDs and
  ...
  house style and emits `data/jokes-manifest.json` / `data/quotes-manifest.json` with normalized hashes while preserving any
  existing embedding metadata. If the datasets are unchanged, the manifest keeps its previous `meta.generatedAt` timestamp so reruns
  avoid noisy diffs.
